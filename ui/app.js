...
function openGCRoom(el, roomId, roomName) {
  const msgs = roomId === 'hub' ? GC.hubMsgs : (GC.groups.find(g => g.id === roomId)?.msgs || []);
  const myName = state.profile.displayName || state.user?.username || 'You';
  ...
  el.innerHTML = `
    <div class="dm-wrap">
      ...
      <div class="dm-messages" id="gc-room-msgs">
        ${msgs.map(m => m.isSystem
          ? `<div class="gc-system-msg">${m.content}</div>`
          : `<div class="dm-msg ${m.isOwn ? 'own' : 'other'}">
              ${!m.isOwn ? `<span class="dm-sender">${m.sender}</span>` : ''}
              <div class="dm-bubble">${m.content}</div>
              <span class="dm-ts">${m.ts}</span>
            </div>`
        ).join('')}
      </div>
      <div class="dm-input-row" style="padding:10px 14px">
  ...
  function doSend() {
    const content = input.value.trim();
    if (!content) return;
    const msg = { sender: myName, content, ts: 'just now', isOwn: true };
    msgs.push(msg);
    input.value = '';
    const div = document.createElement('div');
    div.className = 'dm-msg own';
    div.innerHTML = `<div class="dm-bubble">${content}</div><span class="dm-ts">just now</span>`;
    msgEl.appendChild(div);
    msgEl.scrollTop = msgEl.scrollHeight;
  }
